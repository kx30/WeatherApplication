...
class WeatherPresenter(context: Context): MvpPresenter<WeatherView>() {
    ...
    init {
        ...
        val city = CityManager().getLocation(context)
        Log.d(TAG, "$city")

//        val currentTemperature = CityManager().getLocation(context)!!.toFloat()
//        viewState.setCurrentTemperature(currentTemperature)
    }
    ...
    private fun downloadData() {
        ...
        val disposable = jsonApi.getCurrentWeather()
            ...
            .subscribe({ content ->
                ...
                viewState.setCurrentTemperature(weatherInfo.temperatureCelsius)
            }, {
                it.printStackTrace()
            })
